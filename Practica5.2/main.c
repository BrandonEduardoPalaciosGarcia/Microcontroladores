...
char dir='A';
int P=0;
float dis,circ=9.4247,disa,decimal;
...
void main(void) {
    ...
    while(1){
        dis=(circ/18)*P;//Calculamos distancia a partir de la circunferencia de la rueda y la posición
...
void __interrupt () ISR(void){
        if (IOCBF0==1){//Se activa interrupción de Encoder
            if(dir=='A'){//Si  dir es A la Posición aumenta hacia adelante y hacia atrás decrementa
            ...
            if(dir=='B'){//Si  dir es B la Posición aumenta hacia atrás y hacia adelante decrementa
            if (RB1==0){//Verificamos si va hacia adelante o hacia atrás
                P++;
            }
            if(RB1==1){
                P--;
...
void Cambio(void){//Función para cambiar Displays
    switch(y){
        ...
        break;
        case 2:
            LATD=0b11110111;//Enciende Display 2
        break;
        case 3:
            LATD=0b10101111;//Enciende Display 3 y punto decimal
        break;
        case 4:
